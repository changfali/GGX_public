...
def generateSurface(img_size):
    '''
        input:
            img_size: [h, w]
        output:
            SurfaceArray: [h,w,3], -1~1
    '''
    XsurfaceArray = torch.unsqueeze(torch.linspace(-1.0, 1.0, img_size[1]).repeat(img_size[0], 1), axis=-1)
    YsurfaceArray = torch.unsqueeze(torch.linspace(-1.0, 1.0, img_size[0]).repeat(img_size[1], 1).T, axis=-1)
    surfaceArray = torch.cat([XsurfaceArray, YsurfaceArray, torch.zeros(img_size+[1], dtype=torch.float32)], axis=-1)
    return surfaceArray
